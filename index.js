...
// ======================================================
// КОМАНДА /pdf30 — PDF-отчёт за последние 30 дней (ТОЛЬКО АДМИН)
// ======================================================
...
bot.command("pdf30", async ctx => {
  ...
  progressSnap.forEach(doc => {
    ...
    if (!usersActivity[uid]) {
      usersActivity[uid] = { ok: 0, fail: 0, name: p.name || "Без имени" };
    }

    if (p.result === "OK") {
      usersActivity[uid].ok++;
      totalCorrect++;
    } else {
      usersActivity[uid].fail++;
      totalWrong++;
    }
  });
  ...
  const topUsers = Object.entries(usersActivity)
    .map(([_, d]) => ({
      name: d.name,
      total: d.ok + d.fail,
      ok: d.ok,
  ...
  await createPDF(filePath, stats);

  await ctx.replyWithDocument({ source: filePath });

  ctx.reply("✔ PDF отправлен.");
});
